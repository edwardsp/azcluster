...
### Fixed
- **`azhealthcheck` `kmsg` check false positive.** On a clean node the check reported `ERROR kmsg: 1 critical kernel message(s) in last hour` with the finding shown as `-- No entries --`. Root cause: `journalctl --dmesg --priority=crit --since "1 hour ago"` prints the literal placeholder line `-- No entries --` when nothing matches, and the check counted every non-empty stdout line as a critical message. Added `--quiet` to the invocation and a filter that drops journalctl meta lines (`-- … --`) so an empty result is correctly reported as OK. Benign Azure ND noise (PCIe DOE mailbox probe failures, empty `sr0` virtual-CD-ROM reads) is `err`-level and below the `crit` threshold, so it was never the trigger — the placeholder line was.

### Changed
- **Rich v0.25.0 walkthrough docs with per-GPU charts.** [`doc/full-walkthrough-slurm-v0.25.0.md`](doc/full-walkthrough-slurm-v0.25.0.md) and [`doc/full-walkthrough-aks-v0.25.0.md`](doc/full-walkthrough-aks-v0.25.0.md) were rewritten from thin stubs into full per-stage walkthroughs with per-job GPU panels, captured on a fresh fully-instrumented matched re-run (`cmpsl8` + `cmpaks8`, 2× ND H200 v5 / mexicocentral, 2026-06-19). Panels are rendered from **per-node `nvidia-smi` sampled directly on the GPUs** (power/temp/GPU-util/mem-BW-util for all 16 GPUs, identical capture on both stacks) rather than the AMW — this guarantees every panel is populated and the data survives cluster teardown (an earlier AMW-only attempt produced an empty SM-active panel that could not be regenerated after the AMW was deleted). The AKS doc includes a **blobcache-over-InfiniBand graph**: of the 688 GB DeepSeek-R1 load, node-0 pulled the full copy from Blob and **node-1 fetched its entire 708.6 GB copy (100%) over IB RDMA** from node-0 (3.6 MB from Blob). Terminology corrected throughout: Azure offers no literal "bare metal" here — both NCCL rows run on the *same* ND H200 VM, the axis is **non-containerized (HPC-X on the VM) vs containerized (Pyxis/Enroot)**. The `NCCL_TOPO_FILE` requirement is framed as a required multi-node setting (auto-set on Slurm via the HPC image, manually mounted on AKS), not a Slurm-vs-AKS architectural difference.
...
### Added
- **AKS managed Prometheus observability (M5)**: `azcluster deploy --target aks` now enables AKS managed Prometheus against the reused Azure Monitor Workspace when monitoring is on, creating the DCE/DCR/DCRA chain that links AKS metrics ingestion to `amw-<cluster>` and applying a resilient `azmonitoring.coreos.com/v1` ServiceMonitor for NVIDIA DCGM in `gpu-operator`. GPU/IB metrics flow into the existing AMW and are visible through the existing Azure Managed Grafana; pass `--no-monitoring` to skip AMW/AMG and the ServiceMonitor stage.
- **Native AKS Kubernetes operate client**: `azcluster exec`, `logs`, and `ssh --host <node>` now use a hand-rolled Rust Kubernetes client instead of shelling out to laptop `kubectl`. The client fetches AKS cluster-admin credentials via ARM, parses kubeconfig YAML, builds rustls-backed HTTP + WebSocket TLS surfaces from the embedded CA/client cert/key, streams pod logs, speaks the Kubernetes `v5.channel.k8s.io` exec/attach protocol (TTY + SIGWINCH resize + exit-code propagation), and creates a privileged `kubectl debug node/<node>`-style host-root shell (hostPath `/` + `chroot`). No `kube-rs`/`k8s-openapi` dependency was added, preserving Rust 1.78 MSRV (every published `kube` release requires ≥1.82). `azcluster tunnel` for AKS is deferred — the Kubernetes WebSocket port-forward uses the `SPDY/3.1+portforward.k8s.io` tunneling subprotocol (a SPDY framing layer), tracked as a follow-up; it bails with the interim `kubectl port-forward` command.
...
- **AKS local-NVMe storage + `azcluster kubeconfig` + storage examples**: `azcluster deploy --target aks` installs **Azure Container Storage's `local-csi-driver`** (Helm `oci://localcsidriver.azurecr.io/acstor/charts/local-csi-driver` v0.2.13) as a storage-gated `runCommand` stage and applies the `local-csi` StorageClass — the default LVM-RAID auto-stripes a RAID-0 across all 8 ND H200 NVMe Direct Disks (~28.6 TiB/node), consumed via generic ephemeral volumes (no `hostPath`). New `azcluster kubeconfig <name>` fetches the cluster-admin kubeconfig via ARM `listClusterAdminCredential` → `~/.azcluster/kube/<name>.config`. `azcluster status` now surfaces the storage account, data container URL, and kubelet identity client id (new `kubeletIdentityClientId` Bicep output threaded into `AksState`). The storage **examples** ship as repo manifests under [`examples/aks/`](examples/aks/) — deliberately **not** embedded in the binary: `azcp-upload.yaml` (stage data to Blob via IMDS managed identity), `blobcache-rdma.yaml` (2-node blobcached StatefulSet doing peer chunk reads **over InfiniBand / UCX-RDMA**, cache on ACStor NVMe), `training-blobcache.yaml` (PyTorchJob with a per-job blobcache sidecar + azcp checkpoint-save sidecar). The binary provisions infrastructure; benchmarks/examples are run with `kubectl`/`helm`.

### Changed
- **Consolidated Walkthrough Plan**: Replaced the target-specific plan documents with a single, version-agnostic end-to-end plan in [`doc/walkthrough-plan.md`](doc/walkthrough-plan.md). The new plan references runnable files under `examples/` instead of inlining scripts, ensuring it stays current as workloads evolve.
- **Controlled Comparison (v0.25.0)**: Added [`doc/full-walkthrough-slurm-v0.25.0.md`](doc/full-walkthrough-slurm-v0.25.0.md) and updated [`doc/full-walkthrough-aks-v0.25.0.md`](doc/full-walkthrough-aks-v0.25.0.md) to provide a matched-params comparison between Slurm and AKS on 2× ND H200 hardware in `mexicocentral`. Validated that NCCL (~485 GB/s), training (~500+ TFLOP/s/GPU), and vLLM (~9900 tok/s) match within ~2% between both targets.

### Removed
- **Workloads de-embedded**: the NCCL, Megatron-Bridge training, vLLM, and SGLang benchmarks now live as runnable matched files under `examples/slurm/` and `examples/aks/` (identical container images/models/params on both targets for a controlled comparison; plain-VM NCCL is Slurm-only). Removed the `azcluster train` and `azcluster validate` subcommands and stopped cloud-init from writing `/shared/examples` sbatch scripts; `azcluster` now provisions infrastructure only.

### Live-validated
- **AKS storage pipeline on 2× Standard_ND96isr_H200_v5 / mexicocentral**: ACStor `local-csi` RAID-0 of 8 NVMe (~28.6 TiB/node); `azcp` upload to Blob via kubelet managed identity; per-job blobcache FUSE mount + `/hydrate` onto ACStor NVMe (381 MiB/s) + cached read of 100 MiB in 0.1 s; **blobcache peer reads over InfiniBand** — node-1 peer-fetched 26 chunks (~104 MiB) from node-0 over RDMA/UCX (`blobcache_chunk_peer_fetch_seconds_count=26`, node-0 `blobcache_peer_chunk_bytes_served_total=109051904`), Blob touched only for the origin shard; azcp checkpoint-save `.DONE`/`.UPLOADED` handshake over a shared ACStor `/scratch`. blobcache pods are privileged so they reach `/dev/infiniband` without taking the exclusive `rdma/ib` device resource (all 8 HCAs stay free for NCCL).
- **AKS walkthrough-parity runs on 2× ND H200 / mexicocentral** (mirroring the Slurm walkthrough), shipped as `examples/aks/` manifests: **run 6** Llama-3.1-8B-FP8 vLLM served from blobcache = **9,840 tok/s output, 12.66 ms median TPOT, 67.97 ms TTFT** (matches Slurm ~9,863); **run 8** DGXC Megatron-Bridge training **503.5 MODEL_TFLOP/s/GPU**; **run 7** DeepSeek-R1-0528 (640 GB) SGLang TP=16 across both nodes — blobcache distributed the full model over InfiniBand (node-0 cache 688 GB, node-1 peer-fetched 26,934 chunks over RDMA), all 163 shards loaded, TP=16 served 640 requests (headline tok/s pending the warm-server bench; DeepGEMM first-run JIT is the 20-40 min long pole); **run 9** observability — DCGM exporter → AKS managed Prometheus → AMW validated (`count(DCGM_FI_DEV_GPU_UTIL)`=16) via an `azmonitoring.coreos.com` ServiceMonitor. Inference examples (`inference-vllm.yaml`, `inference-sglang-multinode.yaml`) added under `examples/aks/`.
- **Examples/AKS training end-to-end on 2× Standard_ND96isr_H200_v5 / mexicocentral**: git-free training-operator install (pytorch-only, **0 restarts**) from `examples/aks/training-operator.yaml` → `examples/aks/training-megatron-pytorchjob.yaml` submit → steady-state **502 MODEL_TFLOP/s/GPU** (16 GPUs / 2 nodes, gbs=256, CP=2, seq 8192), ~7.5 min wall-clock, matching the manual run and the Slurm H100 DGXC baseline.
- First clean end-to-end `azcluster deploy --target aks` + `examples/aks/nccl-allreduce-mpijob.yaml` on **2× Standard_ND96isr_H200_v5 / mexicocentral**: NCCL all-reduce **481.5 GB/s avg busbw** at 16 GiB / 16 ranks, all 8 `mlx5_0..7` IB/SHARP devices, no TCP fallback — matching/exceeding the Slurm H100 baseline (440 GB/s). Bugs found & fixed live are documented in `AGENTS.md` (K8s version pin, `gpuProfile.driver`, runCommand 202/`/bin/sh`, NFD PCI-class match + GPU-pool taint, `mlx5_*` device naming, worker-DNS race, `/dev/shm`, `all_reduce_perf_mpi` env).
- **Full AKS walkthrough on 2× Standard_ND96isr_H200_v5 / mexicocentral (cluster `aksm5`)**: end-to-end with the native operate client + M5 monitoring on the committed build. `deploy --target aks` (30 resources / 959 s incl. the M5 DCE/DCR/DCRA + DCGM ServiceMonitor stage); native `exec`/`logs`/`ssh --host <node>` (host-root `chroot` shell, clean exit + pod cleanup); example NCCL **483.36 GB/s** avg busbw, 8 IB/SHARP devices/node, no TCP fallback; example training **506.4 MODEL_TFLOP/s/GPU** (16 GPUs/2 nodes); storage azcp upload **15.91 Gbps** + blobcache IB peer-fetch **4.54 GB** over RDMA; vLLM Llama-3.1-8B-FP8 **9,912 tok/s output / 12.55 ms median TPOT / 64 ms TTFT** (1280 reqs); DeepSeek-R1-0528 SGLang TP=16 across both nodes (640 GB hydrated over IB) **1,258.84 tok/s output / 47.92 ms median TPOT / 184.56 ms TTFT** (640/640 reqs); observability `count(DCGM_FI_DEV_GPU_UTIL)`=**16** in the AMW.
...
- **`examples/slurm/{training-megatron,inference-sglang}.sbatch` reported FAILED on successful runs.** Megatron-Bridge SIGTERMs its rank processes on teardown (srun rc=1) and multi-node SGLang SIGKILLs the non-head rank's server after the head finishes the benchmark (srun rc=137) — both *after* the metric is emitted. The examples now capture the metric, treat "reached the final iteration" / "benchmark result emitted" as success, print a `TRAIN_OK`/`SGLANG_OK` summary, and exit 0 (live-validated: training reaches `iteration 50/50` → `TRAIN_OK`, clean exit).
- **`examples/aks/inference-sglang-multinode.yaml` was missing `NCCL_TOPO_FILE` — DeepSeek TP=16 decode was ~20% slow.** Without the NDv5 topology, NCCL builds a generic GPU↔NIC↔NVLink graph and the latency-bound per-token all-reduce on 2× ND H200 runs at **1,339 tok/s / 44.6 ms TPOT**; with the topology it reaches **1,664 tok/s / 36.6 ms**, matching/exceeding the Slurm baseline (1,536 — Slurm gets the topo for free via enroot). AKS nodes don't ship the topology, so the example now mounts the committed `examples/aks/ndv5-topo.xml` via an `ndv5-topo` ConfigMap at `/etc/topology` and sets `NCCL_TOPO_FILE` + `NCCL_IB_PCI_RELAXED_ORDERING=1` + `NCCL_MIN_NCHANNELS=32`. Root-caused on live H200 (AKS 1,339→1,664 by adding the topo; Slurm `NCCL_IGNORE_CPU_AFFINITY=1` unchanged at 1,539, proving the benefit is NCCL channel/routing not CPU pinning). Note: the topo busids must stay 4-hex (`0001:00:00`) as NCCL reports them — `nvidia-smi`'s 8-hex display form is silently ignored by NCCL. See `doc/walkthrough-plan.md` and the AGENTS gotcha.
...
### Fixed
- **Released Linux binary now runs on Ubuntu 22.04 (glibc 2.35).** The `build` job's Linux matrix entry in `.github/workflows/release.yml` was pinned from `ubuntu-24.04` (glibc 2.39) to `ubuntu-22.04` (glibc 2.35). The v0.24.13 `azcluster-cli-*-x86_64-linux.tar.gz` failed at runtime on any host older than 24.04 with `libc.so.6: version GLIBC_2.38 not found / GLIBC_2.39 not found` because GitHub's `ubuntu-latest` rolled to 24.04 sometime between v0.24.0 and v0.24.13 and the workflow followed it. Pinning to 22.04 produces binaries that link against versioned glibc symbols ≤ 2.35, which run on 22.04, 24.04, RHEL 9, Debian 12, and every newer mainstream distro (glibc is backward-compatible — a binary built against an older minor version runs on any newer one). `azcluster-server` and `azhealthcheck` (also in the `build` job) get the same fix. The `assets`, `pyxis`, and `release` jobs intentionally stay on `ubuntu-24.04`: `assets` ships Bicep JSON only (libc-independent); `pyxis` builds `spank_pyxis.so` that is dlopened by slurmd inside the cluster's `microsoft-dsvm:ubuntu-hpc:2404` image (glibc 2.39), where matching 24.04 is correct; `release` only downloads + uploads artifacts.

### Changed
- **Grafana dashboard import moved server-side (issue #1).** Previously `azcluster deploy` finalised by POSTing the four bundled dashboards to AMG from the operator's laptop, which required the deployer principal to hold `Grafana Admin` on the AMG and forced the CLI to absorb the well-known 5–20 min Grafana Admin RBAC propagation lag (long `==> waiting for Grafana Admin role propagation` retry loop). It also blocked headless deploys (CI, agents) where the deployer principal does not hold Grafana Admin. The import now runs on the scheduler VM via a one-shot systemd unit (`azcluster-grafana-import.service`) that POSTs to `/api/folders` + `/api/dashboards/db` using an IMDS token for the monitoring UAI, which holds `Grafana Admin` on the AMG (granted by a new `raMonUaiGrafanaAdmin` role assignment in `bicep/modules/monitoring.bicep`). The importer is idempotent (409 on existing folder is treated as success, dashboard POST uses `overwrite: true`) and retries on 401/403/5xx with a 20-attempt × 60 s budget to absorb the same RBAC propagation lag inside the cluster instead of in the operator's terminal. Cloud-init fetches a new versioned release artifact `azcluster-dashboards-${VERSION}.tar.gz` (published by the `assets` job in `.github/workflows/release.yml`) alongside the existing `azcluster-server-*` tarball and verifies it against the same `SHA256SUMS`. The CLI's `finalize_deploy` no longer touches Grafana at all; the `import_dashboards`, `ensure_grafana_folder`, `get_grafana_token` helpers and the `DASHBOARDS` constant were removed. Operator fallback: `ssh scheduler && sudo systemctl restart azcluster-grafana-import` or read `/var/log/azcluster/grafana-import.log`.
