...
- **AKS training benchmark (M2)**: `azcluster train <aks-cluster> [--nodes N] [--iters N] [--gbs N] [--cp N] [--wait]` submits a Llama-3.1-8B Megatron-Bridge pretraining run as a Kubeflow `PyTorchJob` (torchrun multi-node rendezvous) and, with `--wait`, blocks until the run reaches steady state and reports `MODEL_TFLOP/s/GPU`. The training-operator is installed on demand (idempotent) from a **trimmed, git-free** embedded manifest (`crates/azcluster-cli/src/aks/manifests/training-operator.yaml`): operator + RBAC + self-cert Secret + a minimal preserve-unknown-fields PyTorchJob CRD, base64-inlined through `runCommand` (the upstream `kubectl apply -k github.com/...` needs `git`, absent in the runCommand pod). The operator runs with `--enable-scheme=pytorchjob` only so it never starts an MPIJob controller that would collide with M1's standalone mpi-operator (`mpijobs.kubeflow.org` at `v2beta1`). The launcher (`megatron-pretrain.py`) builds the recipe from the non-gated `NousResearch/Meta-Llama-3.1-8B` config mirror with `load_weights=False`, a NullTokenizer, and mock data — so no HuggingFace token or dataset staging is needed.
...
- **`azcluster train --wait` end-to-end on 2× Standard_ND96isr_H200_v5 / mexicocentral**: fully-automated git-free training-operator install (pytorch-only, **0 restarts**) → PyTorchJob submit → steady-state **502 MODEL_TFLOP/s/GPU** (16 GPUs / 2 nodes, gbs=256, CP=2, seq 8192), ~7.5 min wall-clock, matching the manual run and the Slurm H100 DGXC baseline. The blocking wait is a re-invokable bounded poll (each `runCommand` exits well under the ~10 min server-side timeout) so it survives both cold-start NGC image pulls and the fact that the Megatron-Bridge master pod does not transition to `Succeeded` after the final iteration.
