...
- **Native AKS Kubernetes operate client**: `azcluster exec`, `logs`, `tunnel`, and `ssh --host <node>` now use a hand-rolled Rust Kubernetes client instead of shelling out to laptop `kubectl`. The client fetches AKS cluster-admin credentials via ARM, parses kubeconfig YAML, builds rustls-backed HTTP + WebSocket TLS surfaces from the embedded CA/client cert/key, streams pod logs, speaks the Kubernetes `v5.channel.k8s.io` exec/attach protocol, supports single-pod `portforward.k8s.io` to arbitrary pod ports via `--remote-port`, and creates a privileged `kubectl debug node/<node>`-style nsenter pod for node-root shells. No `kube-rs`/`k8s-openapi` dependency was added, preserving Rust 1.78 MSRV.
