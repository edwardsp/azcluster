...
- **`--no-monitoring` / `--no-accounting`**: monitoring + accounting default to ON. Pass these flags to skip them during rapid test deploys that don't depend on observability or accounting. Accounting is currently reserved (v0.13.x lands managed MySQL + slurmdbd).
