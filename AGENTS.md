...
- **PMIx 4 vs 5 ABI incompatibility breaks cross-node containerised MPI on Slurm 25.11** (v0.13.4 caveat, confirmed-no-quick-fix). Slurm 25.11 from `packages.microsoft.com/repos/slurm-ubuntu-noble` only ships `mpi_pmix_v4.so` linked against PMIx 4.2.9 (`libpmix.so.2.9.5`). Most current AI/ML containers (e.g. NVIDIA NGC 25.x, the `ai-infrastructure-on-azure/nccl-test` image) ship PMIx 5.x (`libpmix.so.2.13.x`). Cross-node `srun --container-image=... --mpi=pmix` silently produces isolated single-node MPI worlds. Live `apt-cache search` on the MS slurm repo confirms NO `slurm-smd-mpi-pmix-v5` package exists, and the host has only `pmix 4.2.9` available. Workarounds (all v0.14+ scope): (a) bare-metal HPC-X + bind-mount the dataset — THIS IS WHAT THE SHIPPED EXAMPLE USES; (b) container-native `mpirun` over SSH (requires sshd inside the container — the upstream Azure NCCL image doesn't ship it); (c) custom slurm-smd rebuild with `--with-pmix=/path/to/pmix5`; (d) downgrade the container to one that ships PMIx 4. Bind-mounting the host PMIx 4 over the container's PMIx 5 via `LD_LIBRARY_PATH` does NOT work — symbol mismatch still produces isolated worlds.
